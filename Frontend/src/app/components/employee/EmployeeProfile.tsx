    ...
    return (
      <Sidebar role="employee">
        <div className="p-6 max-w-3xl mx-auto">
          <div className="rounded-xl border border-red-500/20 bg-red-500/10 p-4 text-red-700 text-sm flex items-center gap-2">
    ...
    <Sidebar role="employee">
      <div className="p-6 max-w-3xl mx-auto space-y-5">
        ...
        <div className="mb-2">
          ...
          <p className="text-stone-500 text-sm">Manage your personal information and account settings.</p>
        </div>

        {profileError && (
          <div className="rounded-lg border border-red-500/20 bg-red-500/10 px-4 py-2 text-red-700 text-sm">
            {profileError}
          </div>
        )}

        {/* ── Avatar / header card ─────────────────────────────────────────── */}
        <div className="rounded-xl border border-white/10 bg-card p-6 flex items-center gap-5">
          <div className="w-16 h-16 rounded-2xl bg-white/10 border border-white/15 flex items-center justify-center text-2xl font-bold text-white flex-shrink-0">
            {initials}
          </div>
          <div className="flex-1">
            ...
            <p className="text-slate-300 text-sm">{p.email}</p>
            <div className="flex items-center gap-2 mt-2">
              <span className="text-xs px-2 py-0.5 rounded bg-white/10 border border-white/15 text-slate-200 capitalize">
                {p.role}
              </span>
              <span
        ...
        {/* Success banner */}
        {savedOk && (
          <div className="flex items-center gap-2 px-4 py-2.5 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-700 text-sm">
        ...
        <div className="rounded-xl border border-white/10 bg-card p-6">
          <h3 className="text-sm font-semibold text-white mb-5" style={{ fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
          ...
          {editMode ? (
            ...
            <div className="space-y-4">
              {/* Name */}
              <div>
                <label className="block text-xs text-slate-300 mb-1.5 uppercase tracking-wider">Full Name</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-3 rounded-lg border border-white/10 bg-white/5 text-white placeholder:text-slate-500 text-sm focus:outline-none focus:border-slate-400/40 transition"
                  />
                </div>
              </div>
              ...
              {/* Phone */}
              <div>
                <label className="block text-xs text-slate-300 mb-1.5 uppercase tracking-wider">Phone Number</label>
                <div className="relative">
              ...
              {/* Home location */}
              <div>
                <label className="block text-xs text-slate-300 mb-1.5 uppercase tracking-wider">
                  Home Location
                </label>
                {homePos && (
                  <div className="mb-2">
                    <p className="text-sm">
                      <AddressText lat={homePos[0]} lng={homePos[1]} className="text-slate-200" />
                    </p>
                ...
                <div className="grid grid-cols-2 gap-3 mt-3">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="23.8103"
                      className="w-full px-3 py-2 rounded-lg border border-white/10 bg-white/5 text-white placeholder:text-slate-500 text-xs focus:outline-none focus:border-slate-400/40 transition font-mono"
                    />
                  </div>
                  <div>
              ...
              <button
                ...
                disabled={saving}
                className="flex items-center gap-2 px-5 py-2.5 rounded-lg bg-slate-600 hover:bg-slate-500 disabled:opacity-60 text-white font-semibold text-sm transition"
              >
            ...
            <div className="space-y-1">
              {[
                ...
                <div key={label} className="flex items-start gap-3 py-3 border-b border-white/10 last:border-0">
                  <Icon className="w-4 h-4 text-slate-400 mt-0.5 flex-shrink-0" />
                  <div>
                    <p className="text-xs text-slate-300 uppercase tracking-wider mb-0.5">{label}</p>
              ...
              <div className="flex items-start gap-3 py-3 border-b border-white/10 last:border-0">
                ...
                <div>
                  <p className="text-xs text-slate-300 uppercase tracking-wider mb-0.5">Home Location</p>
                  <p className="text-sm">
                    <AddressText lat={p.home_lat} lng={p.home_lng} className="text-slate-200" />
                  </p>
              ...
              {p.home_lat != null && p.home_lng != null && (
                <div className="pt-3">
                  <p className="text-xs text-slate-300 uppercase tracking-wider mb-2">Home on Map</p>
        ...
        <div className="rounded-xl border border-white/10 bg-card p-6">
          ...
          {schedLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-8 text-center">
              ...
              <p className="text-slate-200 text-sm font-medium">No route assigned yet</p>
        ...
        <div className="rounded-xl border border-white/10 bg-card p-6">
          ...
          <form onSubmit={handleChangePassword} className="space-y-4">
            {[
              ...
              <div key={field}>
                <label className="block text-xs text-slate-300 mb-1.5 uppercase tracking-wider">{label}</label>
                <div className="relative">
            ...
            <button
              type="submit"
              className="flex items-center gap-2 px-5 py-2.5 rounded-lg bg-white/10 border border-white/15 hover:bg-white/15 text-white font-semibold text-sm transition"
            >

      ...
      <Sidebar role="employee">
        <div className="p-8 max-w-2xl mx-auto">
          ...
          <div className="flex items-start gap-3 rounded-xl border border-red-500/20 bg-red-500/8 px-5 py-4">
            ...
            <div>
              <p className="text-sm text-red-700">{error ?? 'Could not load the ad-hoc window.'}</p>
              <button onClick={() => window.location.reload()} className="mt-3 text-xs text-amber-600 hover:text-amber-700 font-medium">
                Retry
              </button>
            </div>
          </div>
      ...
      <Sidebar role="employee">
        <div className="p-8 max-w-2xl mx-auto flex flex-col items-center justify-center min-h-[70vh]">
          <div className="rounded-2xl border border-amber-500/20 bg-amber-500/8 p-10 text-center max-w-md">
            ...
            <p className="text-xs text-stone-500 mb-6">Your route updates automatically after 10 PM tonight.</p>
      ...
      <div className="p-6 max-w-6xl mx-auto">
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-1">
            ...
            <h1 className="text-3xl font-bold text-stone-900" style={{ fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
              Ad-hoc Request
            </h1>
          </div>
          <p className="text-stone-500 text-sm ml-11">For same-day transport changes — request only on the day itself, before 7 PM.</p>
        </div>
        ...
        {open ? (
          <div className="flex items-start gap-3 rounded-xl border border-amber-500/15 bg-amber-500/6 px-5 py-4 mb-6">
            ...
            <div className="text-xs text-amber-700/80">
              <p>
                <span className="font-semibold text-amber-700">Ad-hoc is open for today.</span> Submit by{' '}
                <span className="font-semibold text-amber-700">7:00 PM</span> (3 hours before the 10 PM shift). Time left:{' '}
                <span className="font-mono text-amber-700">{fmtCountdown(countdownMs)}</span>
              </p>
              {hasSaved && (
          ...
          <div className="flex items-start gap-3 rounded-xl border border-stone-600/20 bg-stone-800/40 px-5 py-4 mb-6">
            ...
            <div className="text-xs text-stone-600">
              <p>
                <span className="font-semibold text-stone-700">Ad-hoc is closed.</span> {adhoc.reason}
              </p>
            </div>
          </div>
        )}
        ...
        <fieldset disabled={!open} className={open ? '' : 'opacity-60 pointer-events-none'}>
          <div className="rounded-xl border border-white/10 bg-card p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
              <div>
                <label className="block text-xs text-slate-300 mb-2 uppercase tracking-wider">Shift Start Time</label>
                <div className="relative">
                  <Clock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                  <select
                    value={shiftStart}
                    onChange={e => setShiftStart(e.target.value)}
                    className="w-full pl-10 pr-4 py-3 rounded-lg border border-white/10 bg-white/5 text-white text-sm focus:outline-none focus:border-amber-500/40 transition"
                  >
              ...
              <div>
                <label className="block text-xs text-slate-300 mb-2 uppercase tracking-wider">Shift End Time</label>
                <div className="relative">
                  <Clock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                  <select
                    value={shiftEnd}
